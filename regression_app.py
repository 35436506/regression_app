...
def export_to_excel(run_history):
    ...
    for r in run_history:
        ...
        summary_rows.append({
            ...
            'Ridge R²':        rr['r2']          if rr else '',
            'Ridge RMSE':      rr['rmse']        if rr else '',
        })

    sum_df        = pd.DataFrame(summary_rows)
    col_widths    = [8, 22, 32, 24, 8, 13, 13, 13, 13, 15, 11, 11, 13]
    int_sum_cols  = {'Run #', 'N'}
    ...
    for r in run_history:
        ...
        if rr:
            row = write_table(ws, row,
                f"RIDGE REGRESSION (λ={rr['best_lambda']})  ·  R²={rr['r2']}  ·  RMSE={rr['rmse']}",
                rr["coef_df"])
...
<b>④ Khi nào Ridge thực sự giúp ích?</b> Khi VIF cao (>10) và p-value của các biến quan trọng bị thổi phồng — Ridge ổn định hệ số để diễn giải và dự báo đáng tin hơn.
                                    ...
                                    # ── Step 5: Save ridge result into run_history ───────────
                                    ridge_summary = {
                                    ...
                                    # Patch the latest run in session_state
                                    st.session_state["run_history"][-1]["ridge_result"] = ridge_summary
                                    st.success("✅ Kết quả Ridge đã được lưu vào bảng So Sánh Mô Hình bên dưới.")
    ...
    if len(history) > 1:
        with st.expander("🔍 Xem chi tiết từng lần chạy"):
            for r in history:
                ...
                rr = r.get("ridge_result")
                if rr:
                    st.markdown(f"🔷 **Ridge (λ={rr['best_lambda']}):** R² = {rr['r2']} | RMSE = {rr['rmse']} | Biến bị co nhiều nhất: {rr['most_shrunk']}")
                    st.dataframe(rr["coef_df"], use_container_width=True, hide_index=True)
                st.divider()
