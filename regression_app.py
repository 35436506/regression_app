...
def make_plots(model, df_clean, dep_var, ind_vars, model_type):
    ...
    if is_interaction and len(ind_vars) >= 2:
        ...
        mod_levels = {
            f"{mod_var} = low (p10)":   np.percentile(mod_vals, 10),
            f"{mod_var} = mean":        float(mod_vals.mean()),
            f"{mod_var} = high (p90)":  np.percentile(mod_vals, 90),
        }
        ...
        style_ax(ax1,
                 ...
                 key_var, dep_var)
        ax1.annotate(
            f"3 đường = low/mean/high của {mod_var}  |  biến khác giữ ở mean",
...
Biểu đồ Trend vẽ <b>3 đường hồi quy</b> (low/mean/high của X₂) — nếu 3 đường không song song → tương tác có ý nghĩa.<br><br>
...
MODEL_INFO = {
    ...
    "Tương tác (Interaction)": "Thêm tích X₁×X₂ — phù hợp khi ảnh hưởng của X₁ phụ thuộc vào X₂. Biểu đồ Trend vẽ 3 đường theo mức thấp/trung bình/cao của X₂.",
}
...
<b>Lưu ý:</b> Chỉ loại nếu biến đó thực sự không có lý thuyết hỗ trợ. Đừng loại thuần túy vì p-value.
                    ...
                    with sol2:
                        st.markdown('''<div class="info-box" style="font-size:0.85rem;line-height:1.8;">
<b>Ridge Regression</b> thêm hình phạt λΣβ² vào OLS để co hệ số về 0, ổn định hóa ước lượng khi có đa cộng tuyến.<br>
Dữ liệu X được <b>chuẩn hóa (StandardScaler)</b> trước khi fit — hệ số Ridge là hệ số chuẩn hóa, so sánh tầm quan trọng tương đối giữa các biến.
</div>''', unsafe_allow_html=True)

                        # ── Auto-find optimal lambda via RidgeCV ─────────────
                        if st.button("🔍 Tìm λ tối ưu (Cross-Validation)", key=f"ridge_cv_{latest['run_id']}"):
                            try:
                                ...
                                X_r = df_r[latest["ind_vars"]].values
                                y_r = df_r[latest["dep_var"]].values

                                scaler_cv = StandardScaler()
                                X_scaled_cv = scaler_cv.fit_transform(X_r)

                                alphas_cv = np.logspace(-3, 2, 80)   # 0.001 → 100
                                ridge_cv = RidgeCV(alphas=alphas_cv, cv=5)
                                ridge_cv.fit(X_scaled_cv, y_r)
                                best_lam = float(ridge_cv.alpha_)

                                st.session_state[f"ridge_best_lam_{latest['run_id']}"] = best_lam
                                st.markdown(f'''<div class="ok-box">
✅ <b>λ tối ưu (5-fold CV): {best_lam:.4f}</b><br>
Slider bên dưới đã được đặt về giá trị này. Nhấn <b>▶ Chạy Ridge</b> để xem kết quả.
</div>''', unsafe_allow_html=True)
                            except Exception as cv_err:
                                st.error(f"Lỗi RidgeCV: {cv_err}")

                        # ── Lambda slider ──────────────────────────────────────
                        default_lam = st.session_state.get(
                            f"ridge_best_lam_{latest['run_id']}", 1.0)
                        ridge_alpha = st.slider(
                            "Chọn λ (alpha) — càng lớn hệ số càng bị co mạnh:",
                            min_value=0.001, max_value=20.0,
                            value=float(round(min(default_lam, 20.0), 3)),
                            step=0.001, format="%.3f",
                            key=f"ridge_alpha_{latest['run_id']}"
                        )
                        if ridge_alpha > 10:
                            st.warning("⚠ λ > 10: hệ số bị co rất mạnh — mô hình có thể mất nhiều thông tin. Khuyến nghị λ < 10 với hầu hết dataset.")

                        if st.button("▶ Chạy Ridge Regression", key=f"ridge_run_{latest['run_id']}"):
                            try:
                                ...
                                r2_ridge  = r2_score(y_r, y_pred_r)
                                rmse_ridge = float(np.sqrt(np.mean((y_r - y_pred_r) ** 2)))

                                st.markdown(f'''<div class="ok-box">
✅ <b>Ridge (λ={ridge_alpha:.3f}) hoàn thành</b><br>
R² = <b>{r2_ridge:.4f}</b> &nbsp;|&nbsp; RMSE = <b>{rmse_ridge:.4f}</b>
</div>''', unsafe_allow_html=True)

                                # ── Coefficient comparison table OLS vs Ridge ──
                                ols_params = latest.get("params", {})
                                ols_coefs  = []
                                for v in latest["ind_vars"]:
                                    ols_c = ols_params.get(v, float("nan"))
                                    ols_coefs.append(round(float(ols_c), 4) if not np.isnan(ols_c) else "—")

                                ridge_coefs = [round(c, 4) for c in ridge.coef_]

                                # Shrinkage ratio: |Ridge coef| / |OLS coef (std)|
                                from sklearn.preprocessing import StandardScaler as _SS
                                # Re-fit OLS on scaled data to get comparable coefficients
                                import statsmodels.api as _sm
                                _X_s = _sm.add_constant(X_scaled)
                                _ols_s = _sm.OLS(y_r, _X_s).fit()
                                ols_coefs_std = [round(c, 4) for c in _ols_s.params[1:]]  # skip const
                                shrinkage = []
                                for oc, rc in zip(ols_coefs_std, ridge_coefs):
                                    if abs(oc) > 1e-9:
                                        shrinkage.append(f"{abs(rc)/abs(oc)*100:.1f}%")
                                    else:
                                        shrinkage.append("—")

                                coef_ridge = pd.DataFrame({
                                    "Biến": latest["ind_vars"],
                                    "Hệ số OLS (std)":   ols_coefs_std,
                                    "Hệ số Ridge (std)": ridge_coefs,
                                    "Co lại còn":        shrinkage,
                                })
                                st.dataframe(coef_ridge, use_container_width=True, hide_index=True)
                                ...
                                st.markdown(f'''<div class="info-box" style="font-size:0.82rem;line-height:1.75;">
📌 <b>So sánh với OLS:</b> R² OLS = {ols_r2:.4f} → Ridge R² = {r2_ridge:.4f} ({dir_str}).<br>
Ridge thường có R² trong mẫu thấp hơn OLS một chút — đây là bình thường và chấp nhận được.<br><br>

<b>🔍 Nhìn vào đâu để biết đa cộng tuyến được khắc phục?</b><br>
<b>① Cột "Co lại còn":</b> Biến nào bị co nhiều (ví dụ còn 30%) = biến đó bị đa cộng tuyến ảnh hưởng nặng nhất trong OLS. Ridge làm ổn định hệ số của nó.<br>
<b>② Hệ số OLS (std) có dấu bất thường không?</b> Ví dụ: +500 và −480 cho hai biến tương quan cao — Ridge kéo về +20 và +15 → hợp lý hơn nhiều.<br>
<b>③ VIF vẫn cao sau Ridge</b> — đây là bình thường. Ridge <i>không</i> làm giảm VIF (VIF đo tương quan giữa X, Ridge không thay đổi X). Ridge kiểm soát <i>hậu quả</i> của VIF cao: hệ số không còn dao động mạnh nữa.<br>
<b>④ RMSE Ridge ≈ RMSE OLS</b> với R² giảm ít → Ridge đang làm đúng việc: hi sinh một chút R² trong mẫu để đổi lấy hệ số ổn định hơn.<br><br>
Thử tăng/giảm λ để quan sát mức độ co hệ số thay đổi theo cột "Co lại còn".
</div>''', unsafe_allow_html=True)
