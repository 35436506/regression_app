...
Chạy Ridge bên dưới với dữ liệu hiện tại:
</div>''', unsafe_allow_html=True)
...
✅ <b>Ridge (λ={ridge_alpha}) hoàn thành</b><br>
R² = <b>{r2_ridge:.4f}</b> &nbsp;|&nbsp; RMSE = <b>{rmse_ridge:.4f}</b>
                                ...
                                coef_ridge = pd.DataFrame({
                                    ...
                                    "Hệ số Ridge (std)": [round(c, 4) for c in ridge.coef_],
                                    "Diễn giải": ["Tương đối — không so sánh đơn vị gốc" for _ in latest["ind_vars"]]
                                })
                                ...
                                st.markdown(f'''<div class="info-box" style="font-size:0.82rem;">
📌 <b>So sánh với OLS:</b> R² OLS = {ols_r2:.4f} → Ridge R² = {r2_ridge:.4f} ({dir_str}).<br>
Ridge thường có R² trong mẫu thấp hơn OLS một chút, nhưng hệ số ổn định hơn và dự báo ngoài mẫu tốt hơn.<br>
Thử tăng/giảm λ để cân bằng bias–variance.
</div>''', unsafe_allow_html=True)
